...
def remove_ts2_tab_items(file_content):
    """
    Remove TabItem blocks for TypeScript Client v2 from the content.
    This uses a combination of regex and state tracking to properly handle nested tags.
    """
    # Pattern to detect the start of TypeScript Client v2 TabItem
    start_pattern = re.compile(
        r'<TabItem\s+.*?value=([\'"])(ts2|js2|js)\1.*?label=([\'"])JS/TS Client v2\3.*?>'
    )
    ...
    for line in lines:
        # Check if we're starting a TypeScript Client v2 TabItem
...
def process_directory(directory_path, file_extensions, dry_run=True):
    ...
    for ext in file_extensions:
        for file_path in directory.glob(f"**/*{ext}"):
            ...
            # Process the content
            new_content = remove_ts2_tab_items(content)
...
def main():
    parser = argparse.ArgumentParser(
        description="Remove TypeScript Client v2 TabItem blocks from files."
    )
