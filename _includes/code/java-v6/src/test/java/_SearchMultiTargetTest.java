...
class MultiTargetSearchTest {
  ...
  public static void beforeAll() throws IOException, InterruptedException {
    ...
    client = WeaviateClient.connectToWeaviateCloud(
        weaviateUrl,
        weaviateApiKey,
        config -> config.setHeaders(Map.of(
    ...
    CollectionHandle<Map<String, Object>> nvjcCollection = client.collections.use(COLLECTION_NAME);
  ...
  void testMultiBasic() throws Exception {
    ...
    for (var o : response.objects()) {
      System.out.println(objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(o.properties()));
  ...
  void testMultiTargetNearVector() throws Exception {
    ...
    float[] v1 = someResult.objects().get(0).metadata().vectors().get("jeopardy_questions_vector");
    float[] v2 = someResult.objects().get(1).metadata().vectors().get("jeopardy_answers_vector");
    ...
    var response = collection.query.nearVector(
        ...
        q -> q
            .limit(2)
            // No need to specify targetVectors here, it's inferred from the Target objects
  ...
  void testMultiTargetMultipleNearVectors() throws Exception {
    ...
    float[] v1 = someResult.objects().get(0).metadata().vectors().get("jeopardy_questions_vector");
    float[] v2 = someResult.objects().get(1).metadata().vectors().get("jeopardy_answers_vector");
    float[] v3 = someResult.objects().get(2).metadata().vectors().get("jeopardy_answers_vector");
    assertThat(v1).isNotEmpty();
    assertThat(v2).isNotEmpty();
    assertThat(v3).isNotEmpty();

    // START MultiTargetMultipleNearVectorsV1
    // Using List<float[]> for multiple vectors per target
    List<float[]> answerVectors = List.of(v2, v3);

    var responseV1 = collection.query.nearVector(
        // highlight-start
        // Specify the query vectors for each target vector
        Target.vector("jeopardy_questions_vector", v1),
        Target.vector("jeopardy_answers_vector", answerVectors),
        // highlight-end
        q -> q
            .limit(2)
            // Target vectors are inferred
            .returnMetadata(Metadata.DISTANCE)
    );
    ...
    var responseV2 = collection.query.nearVector(
        ...
        Target.manualWeights(
          ...
          Target.vector("jeopardy_answers_vector", List.of(30f, 30f), answerVectors) // Matches the order of the vectors
        ),
        // highlight-end
        q -> q
            .limit(2)
            // Target vectors and weights specified in the Target object
            .returnMetadata(Metadata.DISTANCE)
    );
  ...
  void testMultiTargetWithSimpleJoin() throws Exception {
    ...
    var response = collection.query.nearText(
        "a wild animal",
        // highlight-start
        Target.average("jeopardy_questions_vector", "jeopardy_answers_vector"), // Specify the target vectors and the join strategy
