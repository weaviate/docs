...
for p in config.properties:
    if p.name == "title":
        assclient.collections.use( == "LOWERCASE"
    elif p.name == "body":
...
# END AddNameclient.collections.use(
...
# END Replicaclient.collections.use(
...
# Test
collection = client.collections.use("Article")
config = collclient.collections.use(
...
client.collections.create(
    "Article",
    # highlight-start
    sharding_client.collections.use(ng(
...
collection = client.collections.use("Article")
config = collection.config.get()
assert confclient.collections.use(ual_per_physical == 128
...
# START Multiclient.collections.use(
from weaviate.classes.config import Configure
...
# Clean slate
client.collclient.collections.use(")
