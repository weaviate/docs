...
func TestPQConfiguration(t *testing.T) {
	...
	t.Run("Enable PQ on Existing Collection - Minimal", func(t *testing.T) {
		className := "MyExistingCollectionPQ"
		...
		cfg["pq"] = map[string]interface{}{
			...
			"trainingLimit": 100000, // Optional: number of vectors to use for training
			"segments":      96,     // Optional: number of segments for product quantization
			"centroids":     256,    // Optional: number of centroids (default 256)
			"encoder": map[string]interface{}{
				"type":         "kmeans", // Optional: encoder type (kmeans or tile)
				"distribution": "normal", // Optional: distribution for tile encoder
			},
		}
