...
このデモでは、NVIDIA 上の埋め込みモデルと生成モデルを使用してブログ記事のベクトルを生成し、生成モデルで新しいコンテンツを作成します。
...
3. Weaviate クライアント バージョン `4.11.0` 以降  
4. Weaviate データベース バージョン `1.28.5`、`1.29.0` 以降  
...
[生成モデルの全一覧](https://weaviate.io/developers/weaviate/model-providers/octoai/generative#available-models)
...
ハイブリッド検索は BM25 と ベクトル検索 を組み合わせ、`alpha` パラメーターに応じて両アルゴリズムの重み付けを行います。 
...
以下の処理が行われます。  
1. ベクトルデータベースから関連性の高いチャンクを 3 件取得します  
2. 取得した 3 件を NVIDIA に渡し、Ref2Vec について短い段落を生成します  
