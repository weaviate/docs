...
---
[![Colab で開く](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/weaviate/recipes/blob/main/weaviate-features/model-providers/cohere/similarity_search_embed_multilingual_v2.0.ipynb)
...
## Weaviate への クエリ: 類似検索 (テキストオブジェクト)
...
「movies に登場する animals」についての `JeopardyQuestion` を検索します。返却件数は 4 件に制限します。
...
`JeopardyQuestion` クラス内を検索し、ID `a1dd67f9-bfa7-45e1-b45e-26eb8c52e9a6` に最も近い 4 件を取得します。（この ID は上記のクエリ結果から取得したものです）
...
`JeopardyQuestion` クラス内を検索し、クエリ ベクトル `[-0.0125526935, -0.021168863, ... ]` に最も近い 2 件を取得します。
