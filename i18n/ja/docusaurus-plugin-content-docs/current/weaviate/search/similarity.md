...
ベクトル検索は、クエリのベクトルに最も近いベクトルを持つオブジェクトを返します。

## テキストによる検索

入力テキストに最も近いベクトルを持つオブジェクトを検索するには、`Near Text` オペレーターを使用します。
...
画像に最も近いベクトルを持つオブジェクトを検索するには、[`Near Image`](../api/graphql/search-operators.md) オペレーターを使用します。<br/>
この例では、画像を base64 表現で使用します。
...
詳細は [画像検索](./image.md) を参照してください。
## 既存オブジェクトでの検索

オブジェクト ID がある場合は、[`Near Object`](../api/graphql/search-operators.md#nearobject) オペレーターを使用して、そのオブジェクトに類似したオブジェクトを検索します。
...
<details>
  ...
  <div>
    オブジェクト ID を取得するには、[オブジェクト ID の取得](./basics.md#retrieve-the-object-id) を参照してください。
  </div>
</details>
...
入力ベクトルがある場合は、[`Near Vector`](../api/graphql/search-operators.md#nearvector) オペレーターを使用して、類似したベクトルを持つオブジェクトを検索します。
...
<Tabs groupId="languages">
  ...
  <TabItem value="go" label="Go">
    ...
    サンプルを実行するには、以下のテストベクトルをコード サンプルに貼り付けてください。

<details>
  <summary>サンプル テスト ベクトル</summary>
...
[named vectors](../config-refs/collections.mdx#named-vectors) を持つコレクションを検索するには、検索対象の名前付きベクトルを `target vector` フィールドで指定してください。
...
検索ベクトルと対象ベクトルの間に類似度のしきい値を設定するには、最大 `distance`（または `certainty`）を定義します。
...
- distance の値は、使用するベクトライザー モデルなど、多くの要因に依存します。データで試行し、最適な値を見つけてください。  
- [`certainty`](../config-refs/distances.md#distance-vs-certainty) は `cosine` 距離でのみ使用できます。  
- 類似度が最も低いオブジェクトを探す場合は、`nearVector` 検索で負のコサイン距離を使用してください。  

</details>
## `limit` と `offset`

`limit` を使用して、返却するオブジェクトの最大数を固定します。  
...
クエリとの距離が近いもの同士のグループ単位で結果を制限するには、[`autocut`](../api/graphql/additional-operators.md#autocut) フィルターで返却するグループ数を設定します。
...
プロパティまたはクロスリファレンスを使用して結果をグループ化します。  
返されるオブジェクトをグループ化するには、` Near ` 検索演算子（` Near Text ` や ` Near Object ` など）をクエリに含める必要があります。
...
- 画像検索については [Image search](/weaviate/search/image) をご覧ください。
- チュートリアルは [Queries](/weaviate/tutorials/query.md) を参照してください。
- GraphQL API を利用した検索については [GraphQL API](/weaviate/api) をご覧ください。
