...
export default function NavigationModal({
  ...
  return (
    <BaseModal
      ...
      showCloseButton={false} // We'll use custom header with shortcut
      maxWidth="750px"
    >
      {/* Modal Header */}
      <div className={styles.modalHeader}>
        ...
        <div className={styles.modalHeaderRight}>
          <div
            className={`${buttonStyles.modalHeaderButton} ${styles.headerButton}`}
            onClick={() => setModalOpen(false)}
          >
            <span
              className={`${buttonStyles.buttonShortcut} ${styles.buttonShortcut}`}
            >
              {isApple ? "⌘U" : "Ctrl+U"}
            </span>
            <div
              className={`${buttonStyles.verticalDivider} ${styles.verticalDivider}`}
            ></div>
            <button
              className={`${buttonStyles.headerCloseIcon} ${styles.headerCloseIcon}`}
            >
              ✕
            </button>
          </div>
      ...
      <div className={styles.modalBody}>
        ...
        <div className={styles.aiAssistantSection}>
          ...
          <div className={styles.resourcesGrid}>
            ...
            <div
              className={styles.resourceCard}
              onClick={() => window.open("https://forum.weaviate.io", "_blank")}
            >
              <i
