...
![Single vs Multi-vector embedding comparison visualization](../../docs/weaviate/tutorials/_includes/single_multi_vector_comparison_light.png#gh-light-mode-only "Single vs Multi-vector embedding comparison visualization")
![Single vs Multi-vector embedding comparison visualization](../../docs/weaviate/tutorials/_includes/single_multi_vector_comparison_dark.png#gh-dark-mode-only "Single vs Multi-vector embedding comparison visualization")
...
![ColBERT late interaction vs single-vector visualization](../../docs/weaviate/tutorials/_includes/colbert_late_interaction_light.png#gh-light-mode-only "ColBERT late interaction vs single-vector visualization")
![ColBERT late interaction vs single-vector visualization](../../docs/weaviate/tutorials/_includes/colbert_late_interaction_dark.png#gh-dark-mode-only "ColBERT late interaction vs single-vector visualization")
...
If you would like to try out multi-vector embeddings in Weaviate, check out the [Multi-vector embeddings tutorial](/docs/weaviate/tutorials/multi-vector-embeddings) which will take you end-to-end, for both:

- [Using Jina AI ColBERT model integration](/docs/weaviate/tutorials/multi-vector-embeddings#option-1-colbert-model-integration), or
- [Using user-provided multi-vector embeddings](/docs/weaviate/tutorials/multi-vector-embeddings#option-2-user-provided-embeddings).

## NVIDIA model support

Weaviate's suite of [model integrations](/docs/weaviate/model-providers/) now includes support for NVIDIA's NIM inference service.

![Embedding integration illustration](../../docs/weaviate/model-providers/_includes/integration_nvidia_embedding.png)

Weaviate users can now use NVIDIA model integration to [create text embeddings](/docs/weaviate/model-providers/nvidia/embeddings), [create multi-modal embeddings](/docs/weaviate/model-providers/nvidia/embeddings-multimodal), and use [generative AI models](/docs/weaviate/model-providers/nvidia/generative). (Reranker model support coming soon)
...
Refer to the [RBAC documentation](/docs/weaviate/configuration/rbac) for more information.
...
To read more about BlockMax WAND, and to try it out, refer to the [Indexing page](/docs/weaviate/concepts/indexing/inverted-index#blockmax-wand-algorithm).
...
To activate async replication, set `asyncEnabled` to true in the [`replicationConfig` section of your collection definition](/docs/weaviate/manage-collections/multi-node-setup#replication-settings). Visit the [How-to: Replication](/docs/weaviate/configuration/replication#async-replication-settings) page to learn more about the available async replication settings, and [Concepts: Replication/Consistency](/docs/weaviate/concepts/replication-architecture/consistency) for more information on how async replication works.
...
For those of you upgrading a self-hosted version, please check the [migration guide](/docs/weaviate/more-resources/migration#general-upgrade-instructions) for detailed instructions.
