...
Deactivating unused subsets of data will help to reduce cost and improve the overall performance. We are very excited for you to [try it out](/docs/weaviate/manage-collections/multi-tenancy).

> Read more:
> - <SkipLink href="/docs/weaviate/api/rest#tag/schema/post/schema/%7BclassName%7D/tenants">REST API: Schema: Multi-tenancy</SkipLink>
...
> Read more:
> - [Modules: text2vec-gpt4all](/docs/weaviate/model-providers/gpt4all/embeddings)
...
We’ll explore the awesomeness of this new module more in a blog soon but for now, you can read more about the details of how you can use `multi2vec-bind` in the [documentation](/docs/weaviate/model-providers/imagebind/embeddings-multimodal).

> Read more:
> - [Modules: multi2vec-bind](/docs/weaviate/model-providers/imagebind/embeddings-multimodal)
...
There is now an option to use `pread` for virtual memory access by setting it in the `PERSISTENCE_LSM_ACCESS_STRATEGY` [environment variable](/docs/weaviate/config-refs/env-vars).
...
The current default will remain `mmap` - so if you want to change it, you must set the `PERSISTENCE_LSM_ACCESS_STRATEGY` [environment variable](/docs/weaviate/config-refs/env-vars) to `pread`.
...
> Read more:
> - [References: Configuration: Environment variables](/docs/weaviate/config-refs/env-vars)
...
> Read more:
> - [References: Distance metrics: Implementations](/docs/weaviate/config-refs/distances#distance-implementations-and-optimizations)
