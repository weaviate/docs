...
There are two types of data in a Weaviate cluster: your actual data, and system state information. Earlier releases use a two-phase commit protocol to store schema information. This mechanism doesn't scale well and can leads to locking behavior in multi-node clusters. Starting in 1.25, Weaviate uses Raft to store schema information and cluster state details. Raft ensures your schemas are safe and helps you to reliably scale multi-node clusters for production workloads. The underlying data [storage](/docs/weaviate/concepts/storage) is managed the same way as before - [replication](/docs/weaviate/concepts/replication-architecture) and [sharding](/docs/weaviate/concepts/cluster) continue to safeguard your data while making it available for your applications.

If you are new to Weaviate, you can take immediate advantage of Raft. If you are upgrading a Kubernetes deployment from an earlier version, be sure to review the [migration guide](/docs/weaviate/more-resources/migration/weaviate-1-25) before you upgrade. There is a one-time migration step to update the [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/).
...
import OctoAIDeprecationNote from '/docs/weaviate/model-providers/octoai/_includes/octoai_deprecation.md';
...
To get started using Weaviate with OctoAI all you need is an OctoAI API key that you can get [from here](https://octo.ai/)! Read more about how you can use the [`generative-octoai` integration model](/docs/weaviate/model-providers/octoai/generative) and the [`text2vec-octoai` integration model](/docs/weaviate/model-providers/octoai/embeddings).
...
Learn more about how you can use the integration model [here](/docs/weaviate/model-providers/google/embeddings-multimodal).
...
Some third part services offer more efficient, batch APIs. Starting in 1.25, you can configure Weaviate to [send batches of objects](/docs/weaviate/manage-objects/import#batch-vectorization) to vectorization services that offer batch APIs.
...
[Multi-tenant collections](/docs/weaviate/manage-collections/multi-tenancy) are a useful way to separate and organize data. Earlier version of Weaviate, [batch imports](/docs/weaviate/manage-objects/import) already support multi-tenant operations. However, you have to create the tenants before you import data. This extra step can be inconvenient.

Starting in 1.25, you can configure Weaviate to [create new tenants](/docs/weaviate/manage-collections/multi-tenancy#automatically-add-new-tenants)  during batch imports if the tenants don't already exist. Use automatic multi-tenant creation to streamline your data imports.

## Search improvements

Standalone vector searches use the [`nearText`](/docs/weaviate/search/similarity#search-with-text) and [`nearVector`](/docs/weaviate/search/similarity#search-with-a-vector) similarity operators to fine tune search results. Since [hybrid search](/docs/weaviate/search/hybrid) combines the strengths of vector search and keyword search, many of you asked for this feature in hybrid search too. It's here! The 1.25 release adds the [similarity operators](/docs/weaviate/search/hybrid#vector-search-parameters) to the vector component of hybrid search.

There is another search improvement that the Weaviate community is asking for. Starting in 1.25, the `groupBy` operator is available for [hybrid search](/docs/weaviate/search/hybrid#group-results) and [keyword search](/docs/weaviate/search/bm25#group-results). Use `groupBy` to categorize your search results or to aggregate and summarize the data.
