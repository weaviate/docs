...
Weaviate introduced Product Quantization (PQ) earlier this year. Since then, we've improved how PQ works with your data. In v1.23 we've made it easier to get started. PQ requires a training step. We've heard that the training step was tricky to configure, so we created AutoPQ to take care of the training for you. Just [enable AutoPQ](/docs/weaviate/configuration/compression/pq-compression/#configure-autopq) in your system configuration. Then, any time you enable PQ on a new collection, AutoPQ takes care of training and initializes PQ for you.

We have other improvements too. PQ uses [segments](/docs/weaviate/concepts/vector-quantization/#product-quantization) to compress vectors. In this release we have a new algorithm to determine the optimal segment size for your vectors. You can still set the segment size manually, but you shouldn't have to.
...
* Read more about the `flat` index [here](/docs/weaviate/concepts/vector-index#flat-index).
...
With the `1.23` release, it is easier to use Weaviate with many open-source large language models (LLMs) such as Llama2-70b, CodeLlama-34b or Mistral-7B-Instruct. This is made possible by the [generative-anyscale](/docs/weaviate/model-providers/anyscale/generative) module.
...
* Read more about the `generative-anyscale` module [here](/docs/weaviate/model-providers/anyscale/generative).
...
* Read more about the `Python` client `v4` [here](/docs/weaviate/client-libraries/python).


## Performance improvements

* [Lazy shard loading](/docs/weaviate/concepts/storage#lazy-shard-loading) allows you to start working with your data sooner. After a restart, shards load in the background. If the shard you want to query is already loaded, you can get your results right away. If the shard is not loaded yet, Weaviate prioritizes loading that shard and returns a response when it is ready.

* You can now enable an option to [auto-limit available resources](/docs/weaviate/concepts/resources#limit-available-resources) in Weaviate. In applicable systems, you can set the `LIMIT_RESOURCES` [environment variable](/docs/weaviate/config-refs/env-vars).

## Minor changes

The [`nodes` endpoint](/docs/weaviate/config-refs/nodes) can be used to output information about the nodes in your cluster.
