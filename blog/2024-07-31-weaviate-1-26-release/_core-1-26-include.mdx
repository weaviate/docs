...
When you anticipate using numerical comparisons over ranges of values, enable the new rangeable index in your [collection schema](/weaviate/config-refs/schema). The rangeable index is available for `int`, `number`, and `date` data types.

The rangeable index can be enabled alone or with the [filterable index](/weaviate/concepts/filtering#indexrangefilters). Efficient range filters can be combined with other filters, such as the `Equal` filter, to quickly narrow your searches to the most relevant information.
...
For details, see [async replication](/weaviate/concepts/replication-architecture/consistency#async-replication).
...
To enable the Anthropic integration on a local deployment, add `generative-anthropic` to the `ENABLE_MODULES` [environment variable](/weaviate/config-refs/env-vars), or set the `ENABLE_API_BASED_MODULES` environment variable to `true`.

For details, see the [Anthropic integration page](/weaviate/model-providers/anthropic).

## Bulk enable modules

**Experimental** This feature is experimental and may change in future releases.

To enable all API-based [model provider integrations](/weaviate/model-providers), set the `ENABLE_API_BASED_MODULES` [environment variable](/weaviate/config-refs/env-vars) to `true`.
...
When `ENABLE_API_BASED_MODULES` is `true`, the GraphQL [`Explore`](/weaviate/api/graphql/explore) feature is unavailable.
...
For details, see [multi target vector search](/weaviate/search/multi-vector).

## Tenant states changed

Tenant [activity status](/weaviate/concepts/data#tenant-status) is used to manage resource usage.
...
This feature is only available for self-hosted instances and Weaviate Enterprise Cloud. It is not available for [Weaviate Cloud](/cloud) serverless instances.

For details, see [tenant activity statuses](/weaviate/manage-data/tenant-states#activate-tenant) and the [`s3-offload` module](/weaviate/configuration/tenant-offloading#offload-s3-module).
...
For detailed information on the new async client API, see [Async API](/weaviate/client-libraries/python/async). The async API is very similar to the synchronous client API. The [helper methods](/weaviate/client-libraries/python/async#instantiation-helper-functions) to connect to the aysnc API are analogous to the sync connection methods:

- `use_async_with_local`
- `use_async_with_weaviate_cloud`
- `use_async_with_custom`

For code examples, see [async usage examples](/weaviate/client-libraries/python/async#async-usage-examples).

[FastAPI](https://fastapi.tiangolo.com/) is a popular asynchronous web framework for creating modular, web API microservices. For sample application code that uses the async client with FastAPI, see the [application example](/weaviate/client-libraries/python/async#application-level-example).
...
Weaviate can gather metrics that are compatible with the open-source [Prometheus](https://prometheus.io/docs/introduction/overview/) monitoring and alerting system. The v1.26 Weaviate release adds [new metrics](/weaviate/configuration/monitoring) that monitor aysnc indexing queues.
...
To enable the Korean tokenizer, set the `ENABLE_TOKENIZER_KAGOME_KR` [environment variable](/weaviate/config-refs/env-vars) to `true`, and set the [tokenization method](/weaviate/config-refs/schema/#kagome_kr-tokenization-method) for the relevant properties to `kagome_kr`.
