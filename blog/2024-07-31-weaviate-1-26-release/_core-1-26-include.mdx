...
When you anticipate using numerical comparisons over ranges of values, enable the new rangeable index in your [collection schema](/docs/weaviate/config-refs/schema). The rangeable index is available for `int`, `number`, and `date` data types.

The rangeable index can be enabled alone or with the [filterable index](/docs/weaviate/concepts/filtering#indexrangefilters). Efficient range filters can be combined with other filters, such as the `Equal` filter, to quickly narrow your searches to the most relevant information.
...
For details, see [async replication](/docs/weaviate/concepts/replication-architecture/consistency#async-replication).
...
To enable the Anthropic integration on a local deployment, add `generative-anthropic` to the `ENABLE_MODULES` [environment variable](/docs/weaviate/config-refs/env-vars), or set the `ENABLE_API_BASED_MODULES` environment variable to `true`.

For details, see the [Anthropic integration page](/docs/weaviate/model-providers/anthropic).

## Bulk enable modules

**Experimental** This feature is experimental and may change in future releases.

To enable all API-based [model provider integrations](/docs/weaviate/model-providers), set the `ENABLE_API_BASED_MODULES` [environment variable](/docs/weaviate/config-refs/env-vars) to `true`.
...
When `ENABLE_API_BASED_MODULES` is `true`, the GraphQL [`Explore`](/docs/weaviate/api/graphql/explore) feature is unavailable.
...
When you search multiple vectors, add a list of target vectors to the query instead of a single, named vector. Weaviate automatically combines the results from all of the target search vectors. The mechanism is conceptually similar to the [hybrid fusion](https://weaviate.io/blog/hybrid-search-fusion-algorithms) algorithm in [hybrid search](/docs/weaviate/search/hybrid).
...
For details, see [multi target vector search](/docs/weaviate/search/multi-vector).

## Tenant states changed

Tenant [activity status](/docs/weaviate/concepts/data#tenant-states) is used to manage resource usage.
...
This feature is only available for self-hosted instances and Weaviate Enterprise Cloud. It is not available for [Weaviate Cloud](/docs/cloud) serverless instances.

For details, see [tenant activity statuses](/docs/weaviate/manage-collections/tenant-states#activate-tenant) and the [`s3-offload` module](/docs/weaviate/configuration/tenant-offloading#offload-s3-module).
...
For detailed information on the new async client API, see [Async API](/docs/weaviate/client-libraries/python/async). The async API is very similar to the synchronous client API. The [helper methods](/docs/weaviate/client-libraries/python/async#instantiation-helper-functions) to connect to the aysnc API are analogous to the sync connection methods:

- `use_async_with_local`
- `use_async_with_weaviate_cloud`
- `use_async_with_custom`

For code examples, see [async usage examples](/docs/weaviate/client-libraries/python/async#async-usage-examples).

[FastAPI](https://fastapi.tiangolo.com/) is a popular asynchronous web framework for creating modular, web API microservices. For sample application code that uses the async client with FastAPI, see the [application example](/docs/weaviate/client-libraries/python/async#application-level-example).
...
Weaviate can gather metrics that are compatible with the open-source [Prometheus](https://prometheus.io/docs/introduction/overview/) monitoring and alerting system. The v1.26 Weaviate release adds [new metrics](/docs/weaviate/configuration/monitoring) that monitor aysnc indexing queues.

Use these metrics with Prometheus and [Grafana](https://grafana.com/docs/) to monitor your Weaviate instance.

If you don't see a metric that you need, you can extend Weaviate to add new metrics. To suggest a new metric, or to get started writing your own, see the [contributor guide](/docs/contributor-guide).
...
To enable the Korean tokenizer, set the `ENABLE_TOKENIZER_KAGOME_KR` [environment variable](/docs/weaviate/config-refs/env-vars) to `true`, and set the [tokenization method](/docs/weaviate/config-refs/schema/#kagome_kr-tokenization-method) for the relevant properties to `kagome_kr`.
