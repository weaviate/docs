...
:::tip Read more:
- [Concepts: Vector Indexing # Product Quantization](/weaviate/concepts/vector-index#hnsw-with-product-quantizationpq)
- [How-to configure: Indexes # Vector index (see `pq`)](/weaviate/config-refs/schema/vector-index#weaviates-vector-index)
:::
...
It can be combined with all [vector search operators](/weaviate/api/graphql/get#vector-search-operators): `nearXXX`, `bm25`, and `hybrid`. `autocut` is placed after the operator, at the same level with `limit`/`offset`. It is disabled by default and can be explicitly disabled by setting its value to 0 or a negative number.

:::tip Read more:
- [GraphQL references: Additional operators # Autocut](/weaviate/api/graphql/additional-operators#autocut)
- [How-to search: Similarity search # Autocut](/weaviate/search/similarity#autocut)
- [How-to search: BM25 search # Autocut](/weaviate/search/bm25#autocut)
- [How-to search: Hybrid search # Autocut](/weaviate/search/hybrid#autocut)
:::
...
:::tip See our documentation for more examples
Please refer to [our documentation](/weaviate/search/hybrid#fusion-ranking-method) for more code examples, including for various Weaviate client libraries.
:::
