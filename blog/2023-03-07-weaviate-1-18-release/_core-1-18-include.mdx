...
The [`where` filter](/weaviate/api/graphql/filters) allows you to narrow down your data based on specified conditions. [Search operators](/weaviate/api/graphql/search-operators) are applied to GraphQL queries on the class level. We have added `where` filters to BM25 and hybrid search!
