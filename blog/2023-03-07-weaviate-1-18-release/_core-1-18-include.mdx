...
The [`where` filter](/docs/weaviate/api/graphql/filters#where-filter) allows you to narrow down your data based on specified conditions. [Search operators](/docs/weaviate/api/graphql/search-operators) are applied to GraphQL queries on the class level. We have added `where` filters to BM25 and hybrid search!
...
Version `1.17` supported tunable consistency for [fetching by id](developers/weaviate/api/rest#tag/objects/get/objects/%7BclassName%7D/%7Bid%7D). Weaviate `1.18` adds tunable consistency support for all read and write REST operations, including [batching](/docs/weaviate/api/rest#tag/batch) and [cross-references](developers/weaviate/api/rest#tag/objects/post/objects/%7BclassName%7D/%7Bid%7D/references/%7BpropertyName%7D). GraphQL queries do not support tunable consistency yet.
