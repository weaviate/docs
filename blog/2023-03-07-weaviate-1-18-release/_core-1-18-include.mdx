...
Check out the documentation for more information on how to add filters to [BM25](/docs/weaviate/api/graphql/search-operators#bm25-with-where-filter) and [hybrid](/docs/weaviate/api/graphql/search-operators#hybrid-with-where-filter).
...
For more information on how to update a class using the RESTful API, check out the [documentation](/docs/weaviate/api/rest#tag/schema).
...
Version `1.17` supported tunable consistency for [fetching by id](/docs/weaviate/api/rest#tag/objects/get/objects/%7BclassName%7D/%7Bid%7D). Weaviate `1.18` adds tunable consistency support for all read and write REST operations, including [batching](/docs/weaviate/api/rest#tag/batch) and [cross-references](/docs/weaviate/api/rest#tag/objects/post/objects/%7BclassName%7D/%7Bid%7D/references/%7BpropertyName%7D). GraphQL queries do not support tunable consistency yet.
