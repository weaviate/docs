...
For more information on how to use the `IsNull` filter, check out the [documentation](/weaviate/api/graphql/filters#by-null-state).
...
For more information on how to filter by the property length, check out the [documentation](/weaviate/api/graphql/filters#by-property-length).
...
Weaviate `1.16` unveils the [ref2vec-centroid](/weaviate/modules/ref2vec-centroid) module! Ref2Vec is about representing a data object based on the objects it references. The `ref2vec-centroid` module uses the average, or centroid vector, of the cross-referenced vectors to represent the referencing object.
...
Check the [ref2vec-centroid](/weaviate/modules/ref2vec-centroid) documentation to learn how to work with Ref2Vec.
...
Check the [/v1/nodes](/weaviate/config-refs/nodes) documentation to learn how to use the `Node Status API`.

## More powerful OIDC Auth
![More powerful OIDC Auth](./img/oidc.png)

Security is very, very important. And let's face it - nobody wants to deal with yet another password. That's why we support OpenID Connect (OIDC) [authentication](/weaviate/configuration/authentication) in Weaviate, which users experience as a convenient and secure single sign-on.
...
* [Authentication](/weaviate/configuration/authentication)
* [Authorization](/weaviate/configuration/authorization)
* [Environment variables](/weaviate/installation/docker-compose#environment-variables)
