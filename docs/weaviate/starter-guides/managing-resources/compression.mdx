...
This page discusses compression algorithms. For more on indexes, see [Vector indexing](/docs/weaviate/concepts/vector-index).
...
Starting in v1.22, Weaviate has an optional, [asynchronous indexing](/docs/weaviate/config-refs/schema/vector-index#asynchronous-indexing) feature which effectively speeds up the import process. Consider enabling asynchronous indexing to improve imports.
...
- If you have async indexing and [AutoPQ enabled](/docs/weaviate/configuration/compression/pq-compression#configure-autopq), PQ compression can be enabled anytime. If AutoPQ is not enabled, you should only enable PQ after you have imported enough objects to [train the algorithm](/docs/weaviate/configuration/compression/pq-compression#manually-configure-pq).
...
- [Compression discussion](/docs/weaviate/concepts/vector-quantization)
