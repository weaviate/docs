...
**Instead of simply raising the limit, consider rethinking your architecture**. If you really need to change the limit, use the [`MAXIMUM_ALLOWED_COLLECTIONS_COUNT`](docs/deploy/configuration/env-vars/index.md) environment variable.
:::
...
- **Access control complexity**: [Fine-grained access control](docs/deploy/configuration/authorization.md) must be implemented to ensure data isolation between tenants.
