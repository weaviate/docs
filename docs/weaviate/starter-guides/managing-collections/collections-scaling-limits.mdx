...
**Instead of simply raising the limit, consider rethinking your architecture**. If you really need to change the limit, use the [`MAXIMUM_ALLOWED_COLLECTIONS_COUNT`](/docs/weaviate/config-refs/env-vars) environment variable.
:::
...
- **Access control complexity**: [Fine-grained access control](/docs/weaviate/configuration/authorization) must be implemented to ensure data isolation between tenants.

:::tip Reduce storage costs
You can change the [state of a tenant](/docs/weaviate/starter-guides/managing-resources/tenant-states) into `inactive` (stored locally on disk) or `offloaded` (stored on cloud storage) in order to save resources.

<br />

Find out more about managing multi-tenancy in [How to: Multi-tenancy operations](/docs/weaviate/manage-data/multi-tenancy).
:::
...
- [How-to: Multi-tenancy operations](/docs/weaviate/manage-data/multi-tenancy)
- [How-to: Manage tenant states](/docs/weaviate/manage-data/tenant-states)
