...
Weaviate uses two types of indexes: [vector indexes](../concepts/indexing/vector-index.md) and [inverted indexes](../concepts/indexing.md#inverted-indexes). Vector indexes are used to store and organize vectors for fast vector similarity-based searches. Inverted indexes are used to store data for fast filtering and keyword searches.

The default vector index type is [HNSW](../concepts/indexing/vector-index.md#hierarchical-navigable-small-world-hnsw-index). The other options are [flat](../concepts/indexing/vector-index.md#flat-index), which is suitable for small collections, such as those in a multi-tenancy collection, or [dynamic](../concepts/indexing/vector-index.md#dynamic-index), which starts as a flat index before switching to an HNSW index if its size grows beyond a predetermined threshold.
