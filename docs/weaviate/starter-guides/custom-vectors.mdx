...
When you import data, you can pass pre-existing vectors or generate new ones. "Bring your own vectors" means you supply the vector embeddings when you upload your data. The embeddings you provide can be custom vectors or vectors that are pre-generated using a [model provider](/docs/weaviate/model-providers).
...
Weaviate is open source. You can [run Weaviate](/docs/weaviate/installation) locally, in the cloud, or as a service. If you want to follow this guide without setting up an instance of your own, consider using a free, Sandbox instance in [Weaviate Cloud](/docs/cloud/quickstart).

### Client library

[Client libraries](/docs/weaviate/client-libraries) simplify working with Weaviate. Clients are available for multiple programming languages. This guide provides examples in Python, Typescript, and cURL.
...
This guide uses a Weaviate Cloud instance to host the collection. To [connect to a Weaviate Cloud instance](/docs/cloud/manage-clusters/connect#retrieve-your-api-keys), you need the following information:

- The Weaviate URL
- The Weaviate API key

If you are using a Sandbox instance, the URL and API keys are listed in the [details panel](/docs/cloud/quickstart.mdx#22-connect-to-your-weaviate-cloud-instance) for your instance.
...
Weaviate stores [data](../concepts/data.md) in collections. Each data object in a collection has a set of properties and a vector embedding. Before you import data, you should create a [collection definition](/docs/weaviate/config-refs/schema) to define the data properties for the collection.
...
For more on configurations, see [vectorizer settings](/docs/weaviate/manage-data/collections#specify-vectorizer-settings).
...
If Weaviate has an [integration](/docs/weaviate/model-providers) for the vectorizer that you use to generate your custom vectors, consider adding your vectorizer to the collection definition. If you specify a vectorizer, Weaviate can generate new vectors when it needs them.
...
This [`nearVector`](/docs/weaviate/search/similarity#search-with-a-vector) query supplies a query vector. The query vector is an embedding of the query string, "biology".
...
- [Batch import](/docs/weaviate/manage-data/import)
- [Manage collections](/docs/weaviate/manage-data/collections)
- [Vector similarity search](/docs/weaviate/search/similarity)
