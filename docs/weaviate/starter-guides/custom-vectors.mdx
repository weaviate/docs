...
Weaviate is a vector database. [Vector databases](https://weaviate.io/blog/what-is-a-vector-database) store data objects and vectors that represent those objects. The vector representation is also called an "embedding."
...
Weaviate is open source. You can [run Weaviate](/weaviate/installation) locally, in the cloud, or as a service. If you want to follow this guide without setting up an instance of your own, consider using a free, Sandbox instance in [Weaviate Cloud](/cloud/quickstart).
...
If you are using a Sandbox instance, the URL and API keys are listed in the [details panel](/cloud/quickstart#explore-the-details-panel) for your instance.
...
Weaviate stores [data](../concepts/data.md) in collections. Each data object in a collection has a set of properties and a vector representation. Before you import data, you should create a [collection definition](/weaviate/config-refs/schema) to define the data properties for the collection.
...
For more on configurations, see [vectorizer settings](/weaviate/manage-data/collections#specify-vectorizer-settings).
...
- [Batch import](/weaviate/manage-data/import)
- [Manage collections](/weaviate/manage-data/collections)
- [Vector similarity search](/weaviate/search/similarity)
