...
After running a query in the console, you can copy a Python or TypeScript snippet that reproduces the same call through the [`weaviate-agents`](/docs/agents/query/index.md) client libraries — useful for moving from exploration into application code.

### Limitations

- The Cloud-console Query Agent does **not** support multi-tenant collections. To use the Query Agent with multi-tenancy, call it from a [client library](/docs/agents/query/usage.md#configure-collections).
