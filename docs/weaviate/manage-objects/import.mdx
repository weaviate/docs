...
import GoV6ImportCode from '!!raw-loader!/_includes/code/go-v6/batch_test.go';
...
The Go client (v5) does not support server-side batching; use [manual batching](#manual-batching) instead, or the `Go v6` tab.
...
Use manual (client-side) batching when you want to control the batch size and concurrency yourself, or when using a client that does not yet support server-side batching (such as the Go v5 client). The following example adds objects to the `MyCollection` collection.
