...
To efficiently check if an object with a given [id](../api/graphql/additional-properties.md#id) exists without retrieving it, make a `HEAD` request to the <SkipLink href="/docs/weaviate/api/rest#tag/objects/head/objects/{className}/{id}">`/v1/objects/` REST endpoint</SkipLink>, or use the following client code:
