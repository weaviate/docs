...
<Tabs className="code" groupId="languages">
  ...
  <TabItem value="go6" label="Go v6">
    <FilteredTextBlock
      text={GoV6Code}
      startMarker="// START DeleteMany"
      endMarker="// END DeleteMany"
      language="go"
    />
  </TabItem>
  <TabItem value="java" label="Java">
...
```go
  questions := client.Collections.Use("EphemeralObject")
  res, err := questions.Data.DeleteSelected(ctx, data.DeleteSelected{
    Filter: &filter.Cond{
      ...
      Value:    []string{"asia", "europe"}, // Note the slice value
      // highlight-end
    },
  })
```
...
```go
  ...
  res, err := questions.Data.DeleteSelected(ctx, data.DeleteSelected{
    Filter: &filter.Cond{
      Target: filter.UUID, // The object id ("_id") field
      // highlight-start
      Operator: filter.ContainsAny,
      Value: []string{ // Note the slice value
