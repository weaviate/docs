...
- Your preferred Weaviate [client library](/docs/weaviate/client-libraries) installed.
...
Ensure you are connected to Weaviate with a user possessing sufficient permissions to manage roles. You can achieve this by either using the predefined `root` role during [Weaviate configuration](/docs/weaviate/configuration/rbac/configuration) or by granting a user the [`manage_roles` permission](/docs/weaviate/configuration/rbac/manage-roles-users#role-management-permissions).
