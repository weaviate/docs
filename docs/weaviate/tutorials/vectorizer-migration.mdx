...
This means your **storage usage for this collection will permanently increase**. Due to this limitation, Method A is best suited for temporary testing or when a permanent increase in storage is acceptable, rather than for production migrations where a clean state is desirable.

---

## Choosing the right method

:::Recommendation: Use collection aliases for production

While both methods achieve the goal of switching vectorizers, **we strongly recommend using collection aliases (Method B) for production migrations**. This approach provides a reversible and efficient way to upgrade your embedding models without long-term side effects like increased storage.
