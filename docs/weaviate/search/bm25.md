...
The Go `v6` client calls this setting `KeywordSimilarity` and names the three choices `query.MinimumTokensMatch(n)`, `query.AllTokensMatch`, and `query.AllTokensMatchCross`.
...
`and_cross` is available in the Python client, the C# client from version `1.2.0`, and the Go client from `v6`, but not yet in the TypeScript or Java clients. The Go `v6` client does not check the server version before sending the operator, so see its [known limitations](/weaviate/client-libraries/go/go-v6#known-limitations). The examples below restrict the search to `question` and `answer`, because the `JeopardyQuestion` collection mixes tokenizations across its properties.
