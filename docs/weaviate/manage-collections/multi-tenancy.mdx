...
description: "Enable and manage multi-tenancy with code examples in Python, TypeScript, Go, Java, and C#."
---
...
:::info Multi-tenancy vs namespaces

Multi-tenancy and [namespaces](../configuration/namespaces.mdx) (`v1.38` Preview) are two different features that can be used together. Multi-tenancy isolates **data within one collection** (per-tenant shards). Namespaces isolate **collections across the cluster** (one customer = one namespace of collections). A multi-tenant collection inside a namespace works as expected.
