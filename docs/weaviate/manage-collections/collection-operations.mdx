...
import CSharpCode from "!!raw-loader!/_includes/code/csharp/ManageCollectionsTests.cs";
...
To create a collection, specify at least the collection name. If you don't specify any properties, [`auto-schema`](https://www.google.com/search?q=../config-refs/collections.mdx%23auto-schema) creates them.
...
- **Manually define you data schema**:
  Avoid using the [`auto-schema`](https://www.google.com/search?q=../config-refs/collections.mdx%23auto-schema) feature, instead, manually [define the properties for your collection](https://www.google.com/search?q=%23create-a-collection-and-define-properties).
- **Avoid creating too many collections**:
  Using too many collections can lead to scalability issues like high memory usage and degraded query performance. Instead, consider [using multi-tenancy](https://www.google.com/search?q=/weaviate/manage-collections/multi-tenancy), where a single collection is subdivided into multiple tenants. For more details, see [Starter Guides: Scaling limits with collections](https://www.google.com/search?q=/weaviate/starter-guides/managing-collections/collections-scaling-limits).
  :::
...
- [References: Configuration: Schema](https://www.google.com/search?q=/weaviate/config-refs/collections.mdx)
- <SkipLink href="/weaviate/api/rest\#tag/schema">
    API References: REST: Schema
  </SkipLink>
- [Available data types](https://www.google.com/search?q=../config-refs/datatypes.md)
...
Find out more about the vectorizer and vector index configuration in [Manage collections: Vectorizer and vector index](https://www.google.com/search?q=./vector-config.mdx).
...
To disable [`auto-schema`](https://www.google.com/search?q=../config-refs/collections.mdx%23auto-schema) set `AUTOSCHEMA_ENABLED: 'false'` in your system configuration file.
...
This configuration for [retrieval augmented generation](https://www.google.com/search?q=../search/generative.md) defines the following:
...
For image searches, see [Image search](https://www.google.com/search?q=../search/image.md).
...
You can update a collection definition to change the [mutable collection settings](https://www.google.com/search?q=../config-refs/collections.mdx%23mutability).
...
Various [inverted index parameters](https://www.google.com/search?q=../config-refs/indexing/inverted-index.mdx%23inverted-index-parameters) are configurable for each collection. Some parameters are set at the collection level, while others are set at the property level.
...
- [Manage collections: Vectorizer and vector index](https://www.google.com/search?q=./vector-config.mdx)
- [References: Collection definition](https://www.google.com/search?q=/weaviate/config-refs/collections.mdx)
- [Concepts: Data structure](https://www.google.com/search?q=../concepts/data.md)
