...
From `v1.40`, a definition that sets only the name creates a [collection without a vector](../config-refs/collections.mdx#no-vector).
...
You can add new properties to a collection, but you cannot change an existing one. See [Collection definition: Mutability](../config-refs/collections.mdx#mutability) for the full list of what can change after creation.
