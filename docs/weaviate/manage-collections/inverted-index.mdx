...
You can configure how the inverted index behaves for each collection and property through various parameters that control [indexing behavior](#set-inverted-index-parameters) and [tokenization strategies](#set-tokenization-type-for-property). Proper configuration of these parameters is crucial for optimizing both search performance and storage efficiency.
...
The inverted index in Weaviate can be configured through various parameters at both the property and collection levels:
