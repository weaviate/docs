...
**`gse`** - Language-aware tokenization for Japanese text. Disabled by default. Enable with the `ENABLE_TOKENIZER_GSE` environment variable.

**`gse_ch`** - The same tokenizer with the Chinese dictionary. Use this value, not `gse`, for Chinese text. Disabled by default. Enable with the `ENABLE_TOKENIZER_GSE_CH` environment variable. For Korean text, see the `kagome_kr` option.
