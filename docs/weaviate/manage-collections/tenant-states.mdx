...
* [`HNSW` index (default)](/academy/py/vector_index/hnsw) - uses the vector index in RAM, a **Hot** resource.
* [`Flat` index](/academy/py/vector_index/flat) - uses the vector index on disk, a **Warm** resource.
* [`Dynamic` index](/academy/py/vector_index/dynamic) - starts as a flat index (using a **Warm** resource), then switches to an HNSW index (a **Hot** resource) at a predetermined threshold.
