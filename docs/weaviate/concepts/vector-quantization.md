...
If memory is the main constraint for a very large dataset, you can also consider the disk-based [HFresh index](./indexing/vector-index.md#hfresh-index) as an index-level alternative to compression: it keeps only a compressed centroid index in memory and stores the rest on disk.
