...
- **The `maxConnections` HNSW index setting**. Each object in memory has at most [`maxConnections`](../config-refs/vectorization.mdx) connections per layer. Each of the connections uses 8-10B of memory. Note that the base layer allows for `2 * maxConnections`.
...
For a more accurate calculation, include a factor for the [`maxConnections`](../config-refs/vectorization.mdx) setting instead of multiplying the base requirement by two.
...
- **Reduce the number of [`maxConnections`](../config-refs/vectorization.mdx) in your HNSW index settings**. Each object in memory has up to `maxConnections` connections. Each of those connections uses 8-10B of memory. To reduce the overall memory footprint, reduce `maxConnections`.
...
For optimal search and import performance, all previously imported vectors need to be held in memory. The size of the vector cache is specified by the [`vectorCacheMaxObjects`](../config-refs/vectorization.mdx) parameter in the collection definition. By default this limit is set to one trillion (`1e12`) objects when you create a new collection.
