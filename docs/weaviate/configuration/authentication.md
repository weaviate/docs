...
For Weaviate Cloud (WCD) instances, authentication is pre-configured with OIDC and API key access. You can [authenticate against Weaviate](../connections/connect-cloud.mdx) with your WCD credentials using OIDC, or [with API keys](/docs/wcs/platform/manage-api-keys).
:::
...
1. Use the `resource owner password flow` for trusted applications like [Weaviate Cloud](/docs/wcs/connect.mdx).
