...
To take advantage of the latest developments on the Weaviate Database database, we recommend migrating your codebase to use the [`v4` client API](./index.md).
<br/>
...
If you have an existing codebase and Weaviate Database database that you expect to remain static, we recommend pinning the version in your requirements file (e.g. `requirements.txt`), like so:
