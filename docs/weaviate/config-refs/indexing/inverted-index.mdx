...
Part of `invertedIndexConfig`. Defines named stopword presets at the collection level. Each preset can be a flat list of words, or extend an existing preset with `additions` and `removals`. Properties can then reference a preset by name via [`textAnalyzer.stopwordPreset`](#textanalyzer).
...
When defining a preset as an object (instead of a flat list), the following fields are available:

| Parameter   | Type       | Details                                                                      |
| :---------- | :--------- | :--------------------------------------------------------------------------- |
| `preset`    | `string`   | Name of a base preset to extend (`"en"`, `"none"`, or another custom preset) |
| `additions` | `string[]` | Words to add on top of the base preset                                       |
| `removals`  | `string[]` | Words to remove from the base preset                                         |

The existing [`stopwords`](#stopwords) configuration remains as the default for properties that do not specify a `textAnalyzer.stopwordPreset` override.
...
| :--------------- | :-------- | :------- | :----------------------------------------------------------------------------------------------------------- |
| `text`           | `string`Ï | yes      | The text to tokenize                                                                                         |
| `tokenization`   | `string`  | yes      | Tokenization method (`word`, `lowercase`, `whitespace`, `field`, `gse`, `trigram`, `kagome_ja`, `kagome_kr`) |
...
`POST /v1/schema/{className}/properties/{propertyName}/tokenize` resolves the full analyzer config from an existing property.
