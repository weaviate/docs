...
| :-------------------------------------------- | :-------- | :------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`bm25`](#bm25)                               | `object`  | `{ "k1": 1.2, "b": 0.75 }` | Sets the `k1` and `b` parameters for the BM25 ranking algorithm. See [**BM25 Configuration**](#bm25) below.                                          |
...
| [`stopwordPresets`](#stopwordpresets)         | `object`  | `{}`                       | Named stopword lists that properties can reference through `textAnalyzer.stopwordPreset`. See [**Stopword presets**](#stopwordpresets) below.        |
| [`tokenizerUserDict`](#tokenizeruserdict)     | `array`   | `[]`                       | Per-collection user dictionary for the Japanese and Korean tokenizers. See [**Tokenizer user dictionary**](#tokenizeruserdict) below.                |
| `cleanupIntervalSeconds`                      | `integer` | `60`                       | How often the asynchronous inverted index cleanup runs, in seconds.                                                                                  |
| `usingBlockMaxWAND`                           | `boolean` | `true` for collections created in `v1.30` and later | Whether the collection uses the [BlockMax WAND algorithm](../../concepts/indexing/inverted-index.md#blockmax-wand-algorithm) for BM25 and hybrid searches. Collections created before `v1.30` need [migration](/deploy/migration/weaviate-1-30.md). |
...
They are set per collection. There is no property-level `bm25` setting: a `bm25` object inside a property definition is discarded, so `k1` and `b` always come from the collection.
...
#### `tokenizerUserDict` {#tokenizeruserdict}

Part of `invertedIndexConfig`. Added in `v1.34.1`. Sets a per-collection user dictionary for the Japanese (`kagome_ja`) and Korean (`kagome_kr`) tokenizers, so that terms the shipped dictionary splits incorrectly are segmented the way you choose. No other tokenizer accepts a user dictionary.

<details>
  <summary>Example <code>tokenizerUserDict</code> configuration - JSON object</summary>
...
Each entry sets one tokenizer, and a tokenizer may appear only once. `source` is the term as it appears in the text, and `target` is how that term should be segmented, with a space between tokens. Both fields are required, and each `source` must be unique within the entry.

:::caution Existing data is not re-tokenized
Changing a user dictionary does not re-tokenize data that is already indexed. The new segmentation applies only to objects written after the change. To apply it to existing objects, re-import them.
:::
