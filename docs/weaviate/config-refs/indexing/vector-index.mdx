...
| :------------------- | :------ | :-------- | :--------------------------------------------------------------------------------------------------------- |
| `distance`           | string  | `cosine`  | Distance metric. The metric that measures the distance between two arbitrary vectors.                       |
| `maxPostingSizeKb`   | integer | dynamic   | Maximum allowed size in KB for a posting list. By default, this is computed dynamically.                    |
| `replicas`           | integer | `4`       | Number of posting lists in which a vector will be added.                                                    |
| `rngFactor`          | integer | `10`      | Factor used when adding a vector to a posting list. If too close to a selected posting list, it is skipped. |
