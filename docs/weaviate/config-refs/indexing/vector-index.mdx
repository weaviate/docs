...
- [`HNSW_GEO_INDEX_EF`](/deploy/configuration/env-vars/index.md#HNSW_GEO_INDEX_EF) is a database-level parameter that sets the search depth (`ef`) for the geo index. It applies to every geo property on the instance and cannot be set per collection.
...
| `rq`                    | object  | --      | No         | Enable and configure [rotational quantization (RQ)](../../concepts/vector-quantization.md#rotational-quantization) compression. Whether RQ is enabled and its `bits` value are fixed at collection creation; `rescoreLimit` can be changed later. <br/><br/> For RQ configuration details, see [RQ configuration parameters](#rq-parameters). |
