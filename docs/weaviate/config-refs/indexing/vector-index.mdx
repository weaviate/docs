...
The goal of `dynamic` indexing is to shorten latencies during query time at the cost of a larger memory footprint. If your priority is the opposite — keeping memory low for a very large dataset — consider the [HFresh index](#hfresh-index) instead.
...
| `maxPostingSizeKB` | integer | `48`     | No      | Maximum size in KB for a posting list. Weaviate uses this value along with the vector dimensions to calculate the maximum number of vectors per posting. Min: `8`, Max: `1024`. Best set when you create the collection, as changing it later does not re-partition data that is already indexed. |
...
import DefaultQuantization from '/_includes/feature-notes/default-quantization.mdx';
