...
| Parameter            | Type    | Description                                                                                                                                                                                 | Default | Mutable |
| :------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------ | :------ |
| `distance`           | string  | Distance metric. The metric that measures the distance between two arbitrary vectors. For available distance metrics, see [supported distance metrics](/weaviate/config-refs/distances.md). | TBD     | TBD     |
| `maxPostingSize`     | integer | Maximum number of vectors that can be stored in a single posting list (cluster). Controls the maximum size of each cluster on disk.                                                         | TBD     | TBD     |
| `minPostingSize`     | integer | Minimum number of vectors that should be stored in a single posting list (cluster). Helps maintain efficient cluster sizes.                                                                 | TBD     | TBD     |
| `replicas`           | integer | Number of replicas for each cluster centroid. Higher values improve fault tolerance but increase memory usage.                                                                              | TBD     | TBD     |
| `rngFactor`          | integer | Random number generator factor used during cluster assignment. Affects the randomness in the clustering process.                                                                            | TBD     | TBD     |
| `searchProbe`        | integer | Number of clusters to probe during search. Higher values improve recall but increase query latency. Similar to the `ef` parameter in HNSW.                                                  | TBD     | TBD     |
| `centroidsIndexType` | string  | The index type used for the centroid graph. Can be `hnsw`. Determines how the centroid layer is indexed for efficient cluster selection during search.                                      | TBD     | TBD     |
