...
import JavaV6CodeMultiTenancy from "!!raw-loader!/_includes/code/java-v6/src/test/java/ManageCollectionsMultiTenancyTest.java";
import CSharpCodeMultiTenancy from "!!raw-loader!/_includes/code/csharp/ManageCollectionsMultiTenancyTest.cs";
import TSCodeReference from "!!raw-loader!/_includes/code/config-refs/reference.collections.ts";
import JavaV6CodeReference from "!!raw-loader!/_includes/code/java-v6/src/test/java/ReferenceCollectionsTest.java";
import CSharpCodeReference from "!!raw-loader!/_includes/code/csharp/ReferenceCollectionsTest.cs";
...
| [`objectTtlConfig`](#object-ttl)             | Object | Sets a time to live (TTL) after which the collection's objects expire.                                                 | Disabled                                                                                                                                                                                              | Yes           |
...
<Tabs className="code" groupId="languages">
  ...
  <TabItem value="ts" label="JavaScript/TypeScript">
    <FilteredTextBlock
      text={TSCodeReference}
      startMarker="// START ReferenceFullDefinition"
      endMarker="// END ReferenceFullDefinition"
      language="ts"
    />
  </TabItem>
  <TabItem value="java" label="Java">
    <FilteredTextBlock
      text={JavaV6CodeReference}
      startMarker="// START ReferenceFullDefinition"
      endMarker="// END ReferenceFullDefinition"
      language="java"
    />
...
A description of the collection. This is for your reference and can also provide additional information to the [Query Agent](/query-agent/index.md).
...
<Tabs className="code" groupId="languages">
  ...
  <TabItem value="ts" label="JavaScript/TypeScript">
    <FilteredTextBlock
      text={TSCodeReference}
      startMarker="// START ReferencePropertyOptions"
      endMarker="// END ReferencePropertyOptions"
      language="ts"
    />
  </TabItem>
  <TabItem value="java" label="Java">
...
A property name may also not *end* in one of the following suffixes, because each would collide with an internal index that Weaviate derives from another property:
...
<details>
  <summary>`gse`, `gse_ch` and `trigram` tokenization methods</summary>

For Japanese and Chinese text, we recommend use of the `gse` family or `trigram` tokenization methods. These methods work better with these languages than the other methods as these languages are not easily able to be tokenized using whitespaces.

There are two `gse` values, and they load different dictionaries:

- `gse` uses the Japanese dictionary. Set `ENABLE_TOKENIZER_GSE` to `true` to load it.
- `gse_ch` uses the Chinese dictionary. Set `ENABLE_TOKENIZER_GSE_CH` to `true` to load it.

Use `gse_ch` for Chinese text. `gse` segments Chinese text with the Japanese dictionary, which returns poor tokens without raising an error.

Neither tokenizer is loaded by default, to save resources.
...
| Chinese text | `gse_ch` or `trigram` | Proper word segmentation |
...
More details about the `indexFilterable`, `indexSearchable` and `indexRangeFilters` property parameters, and about the collection-level `invertedIndexConfig` object, can be found in [Reference: Inverted index](./indexing/inverted-index.mdx).
...
You cannot combine both approaches in the same collection. A definition that uses neither creates a [collection without a vector](#no-vector).
...
A collection that sets any of the top-level `vectorizer`, `vectorIndexType` or `vectorIndexConfig` parameters has one vector space for the whole collection. Weaviate fills in the remaining top-level parameters from the server defaults.
...
<Tabs className="code" groupId="languages">
  ...
  <TabItem value="java" label="Java">
    <FilteredTextBlock
      text={JavaV6CodeReference}
      startMarker="// START ReferenceVectorizer"
      endMarker="// END ReferenceVectorizer"
      language="java"
    />
  </TabItem>
  <TabItem value="csharp" label="C#">
...
<Tabs className="code" groupId="languages">
  ...
  <TabItem value="java" label="Java">
    <FilteredTextBlock
      text={JavaV6CodeReference}
      startMarker="// START ReferenceNamedVectors"
      endMarker="// END ReferenceNamedVectors"
      language="java"
    />
  </TabItem>
  <TabItem value="csharp" label="C#">
...
#### The `default` vector name {#default-vector-name}

`default` is a reserved vector name, and it means different things in the two approaches.

**Single vector collections.** `default` is an alias for the collection's one vector space. Reads and writes that target `default` resolve to it, and a write that sends `vectors: {"default": [...]}` is stored there. The alias is not part of the collection definition, so `GET /v1/schema` returns the top-level parameters and no `vectorConfig` entry. You cannot add a named vector called `default` to such a collection.

**Collections that use `vectorConfig`.** `default` is an ordinary named vector with no special behavior. It is the name Weaviate gives the vector that [auto-schema](#auto-schema) creates.

#### Collections without a vector {#no-vector}

:::info Changed in `v1.40`
:::

A collection definition that sets none of `vectorizer`, `vectorIndexType`, `vectorIndexConfig` or `vectorConfig` creates a collection with no vector. Before `v1.40`, the server defaults filled in the top-level parameters, so such a definition produced a single vector collection.

For a collection without a vector:

- `GET /v1/schema/{collection}` returns none of the vector parameters.
- Objects without vectors are stored as usual. A write that carries a vector is rejected with a `422` error.
- Vector search reports that the collection has no vector index.
- You can [add a named vector](../manage-collections/vector-config.mdx#add-new-named-vectors) later.
- You cannot add the top-level parameters later. An update that sets `vectorizer`, `vectorIndexType` or `vectorIndexConfig` is rejected with a `422` error:

```text
a class-level vectorizer or vector index cannot be added through an update, add a named vector instead
```

Existing collections are unchanged, and a definition that does set the top-level parameters still creates a single vector collection.
...
import AsyncConfigCollection from '/_includes/feature-notes/async-config-collection.mdx';

<AsyncConfigCollection/>
...
:::note Values changed in `v1.34.19`, `v1.35.14`, `v1.36.4` and `v1.37.0`
...
<Tabs className="code" groupId="languages">
  ...
  <TabItem value="csharp" label="C#">
    <FilteredTextBlock
      text={CSharpCodeReference}
      startMarker="// START ReferenceReplicationSettings"
      endMarker="// END ReferenceReplicationSettings"
      language="csharp"
    />
  </TabItem>
</Tabs>
...
This code example enables multi-tenancy on a new collection. `autoTenantCreation` and `autoTenantActivation` are set the same way, and can also be changed later. For worked examples of both, see [How-to: Multi-tenancy operations](../manage-collections/multi-tenancy.mdx#automatically-add-new-tenants).
...
`objectTtlConfig` sets an expiration time for the collection's objects. The expiry is measured from the object's creation time, its last update time, or a `date` property that you name.

| Parameter              | Type    | Description                                                                                                                          | Default | Mutable |
| :--------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------ | :------ | :------ |
| `enabled`              | Boolean | If `true`, objects in this collection expire.                                                                                        | `false` | Yes     |
| `deleteOn`             | String  | The base time for the expiry: `_creationTimeUnix`, `_lastUpdateTimeUnix`, or the name of a `date` property.                          | -       | Yes     |
| `defaultTtl`           | Integer | Seconds added to the `deleteOn` value to get the expiration time. Must be positive for `_creationTimeUnix` and `_lastUpdateTimeUnix`. | `0`     | Yes     |
| `filterExpiredObjects` | Boolean | If `true`, expired objects that the background process has not deleted yet are excluded from query results.                          | `false` | Yes     |

Expired objects are only deleted if the [`OBJECTS_TTL_DELETE_SCHEDULE`](/deploy/configuration/env-vars/index.md#OBJECTS_TTL_DELETE_SCHEDULE) environment variable is set on the instance. It is empty by default, which disables the deletion process.

For code examples, see [How-to: Time to live (TTL)](../manage-collections/time-to-live.mdx).
...
| Parameter                            | Mutable after creation?                                            | How                                                                                                                     | If immutable                                                                                                |
| :----------------------------------- | :------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------ |
| `class` (the collection name)        | No                                                                 | -                                                                                                                       | Create a new collection and [migrate the data](../manage-collections/migrate.mdx). To serve an existing collection under another name, use a [collection alias](#collection-aliases). |
| `description`                        | Yes                                                                | [Update the collection definition](../manage-collections/collection-operations.mdx#update-a-collection-definition)       | -                                                                                                           |
| `properties`                         | New properties only                                                | [Add a property](../manage-collections/collection-operations.mdx#add-a-property)                                        | To change an existing property, create a new collection and [migrate the data](../manage-collections/migrate.mdx). |
| `invertedIndexConfig`                | Yes                                                                | [Update the collection definition](../manage-collections/collection-operations.mdx#update-a-collection-definition)       | Changing [`tokenizerUserDict`](./indexing/inverted-index.mdx#tokenizeruserdict) is allowed, but existing objects keep the old tokenization until they are re-imported. |
| `objectTtlConfig`                    | Yes                                                                | [Update the collection definition](../manage-collections/collection-operations.mdx#update-a-collection-definition)       | -                                                                                                           |
| `vectorConfig` (named vectors)       | New named vectors only                                             | [Add new named vectors](../manage-collections/vector-config.mdx#add-new-named-vectors)                                  | To change an existing named vector, create a new collection and [migrate the data](../manage-collections/migrate.mdx). |
| `vectorizer`                         | No                                                                 | -                                                                                                                       | Create a new collection and [migrate the data](../manage-collections/migrate.mdx).                          |
| `vectorIndexType`                    | No                                                                 | -                                                                                                                       | Create a new collection and [migrate the data](../manage-collections/migrate.mdx).                          |
| `vectorIndexConfig`                  | Some settings. See the mutability column in the [vector index reference](./indexing/vector-index.mdx). | [Update the collection definition](../manage-collections/collection-operations.mdx#update-a-collection-definition) | For an immutable setting such as `distance` or `efConstruction`, create a new collection and [migrate the data](../manage-collections/migrate.mdx). |
| `moduleConfig`                       | Generative and reranker modules only, from `v1.26.8` and `v1.27.1` | [Update the collection definition](../manage-collections/collection-operations.mdx#update-a-collection-definition)       | For other module settings, create a new collection and [migrate the data](../manage-collections/migrate.mdx). |
| `replicationConfig.factor`           | No, from `v1.25`                                                   | -                                                                                                                       | See the [replication section](#replication).                                                                |
| `replicationConfig.deletionStrategy` | Yes, from `v1.27.0`                                                | [Update the collection definition](../manage-collections/collection-operations.mdx#update-a-collection-definition)       | -                                                                                                           |
| `multiTenancyConfig.enabled`         | No                                                                 | -                                                                                                                       | Create a new collection with multi-tenancy enabled and [migrate the data](../manage-collections/migrate.mdx). |
| `multiTenancyConfig.autoTenantCreation`, `multiTenancyConfig.autoTenantActivation` | Yes                           | [Update the collection definition](../manage-collections/collection-operations.mdx#update-a-collection-definition)       | -                                                                                                           |
| `shardingConfig`                     | No                                                                 | -                                                                                                                       | Create a new collection and [migrate the data](../manage-collections/migrate.mdx).                          |
...
[`DEFAULT_VECTORIZER_MODULE`](/deploy/configuration/env-vars/index.md#DEFAULT_VECTORIZER_MODULE) is not applied to the `default` vector, so auto-schema does not vectorize your data. Supply the vectors yourself, or define the collection before you import.
...
import CollectionsLimit from '/_includes/feature-notes/collections-limit.mdx';
...
import CollectionAliases from '/_includes/feature-notes/collection-aliases.mdx';
