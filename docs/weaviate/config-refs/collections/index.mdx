...
| :-------------------- | :----- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------- | :---------------- |
| `class`               | String | The name of the collection. See the [class](#class) section below for details.                                                                                   | (Required)                                               | No                |
| `description`         | String | A description of the collection. See the [description](#description) section below for details.                                                                  | `null`                                                   | Yes               |
| `properties`          | Array  | An array of property objects defining the data schema. <br/>See [References: Properties](./properties.mdx) for details.                                          | `[]`                                                     | Partially\*       |
| `vectorConfig`        | Object | Configure named vectors each with their own `vectorizer`, `vectorIndexType`, and `vectorIndexConfig` fields.                                                     | `null`                                                   | Partially\*\*     |
| `vectorizer`          | String | The vectorizer module to use. Set to `"none"` to disable auto-vectorization. <br/>See [References: Vectorization](./vectorization.mdx) for details.              | Module-specific default                                  | No                |
| `vectorIndexType`     | String | The type of vector index to use (`hnsw`, `flat`, `dynamic`). <br/>See [References: Vector index](../indexing/vector-index.mdx) for details.                      | `hnsw`                                                   | No                |
| `vectorIndexConfig`   | Object | Configuration settings specific to the chosen `vectorIndexType`. <br/>See [References: Vector index](../indexing/vector-index.mdx) for details.                  | Index-specific defaults                                  | Partially\*\*\*   |
| `invertedIndexConfig` | Object | Configuration for the inverted index, affecting filtering and keyword search. <br/>See [References: Inverted index](../indexing/inverted-index.mdx) for details. | See [Inverted Index section](#inverted-index-parameters) | Yes               |
| `shardingConfig`      | Object | Controls sharding behavior in a multi-node cluster. <br/>See [References: Cluster configuration](./cluster.mdx) for details.                                     | See [Sharding section](#sharding-parameters)             | No                |
| `replicationConfig`   | Object | Controls data replication settings for fault tolerance. <br/>See [References: Cluster configuration](./cluster.mdx) for details.                                 | `{"factor": 1}`                                          | Partially\*\*\*\* |
| `multiTenancyConfig`  | Object | Configuration to enable multi-tenancy for the collection. <br/>See [References: Multi-tenancy](./multi-tenancy.mdx) for details.                                 | `{"enabled": false}`                                     | Partially**\***   |
| `moduleConfig`        | Object | Module-specific configuration settings.                                                                                                                          | `{}`                                                     | Partially**\*\*** |

\* New properties can be added; existing properties cannot be modified<br/>
\*\* New named vectors can be added; some vector index settings are mutable<br/> \*** See [vector index mutable parameters](#vector-index-mutable-parameters)<br/>
\*\*** See [replication mutable parameters](#replication-parameters)<br/>
\*\***\* See [multi-tenancy mutable parameters](#multi-tenancy-parameters)<br/>
\*\*\*\*** Only generative and reranker modules are mutable (from v1.26.8 and v1.27.1)
...
| Parameter             | Type    | Description                                                                                                                                                                     | Default    | Mutable |
| :-------------------- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :--------- | :------ |
| `name`                | String  | The name of the property. See the [name](#name) section below for details.                                                                                                      | (Required) | No      |
| `dataType`            | Array   | An array containing one or more data types. See [Data Types](../datatypes.md) for a full list. For cross-references, use the capitalized collection name (e.g., `["Article"]`). | (Required) | No      |
| `description`         | String  | A description of the property for your reference.                                                                                                                               | `null`     | Yes     |
| `tokenization`        | String  | For `text` properties, specifies how the text is split into tokens for inverted indexing. See the [tokenization](#tokenization) section below for details.                      | `word`     | No      |
| `indexFilterable`     | Boolean | If `true`, builds a roaring bitmap index for this property to allow for efficient filtering. See [References: Inverted index](../indexing/inverted-index.mdx) for details.      | `true`     | No      |
| `indexSearchable`     | Boolean | If `true`, builds a searchable map index for this property, suitable for BM25 or hybrid search. See [References: Inverted index](../indexing/inverted-index.mdx) for details.   | `true`     | No      |
| `moduleConfig`        | Object  | Module-specific settings, such as skipping vectorization for this property. See [References: Vector index](../indexing/vector-index.mdx) for details.                           | `{}`       | No      |
| `invertedIndexConfig` | Object  | Property-level overrides for inverted index settings, such as `bm25` parameters. See [References: Inverted index](../indexing/inverted-index.mdx) for details.                  | `{}`       | No      |
...
| Parameter                                 | Type   | Description                                                                                                                                                                | Default                 | Mutable       |
| :---------------------------------------- | :----- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------- | :------------ |
| `vectorizer`                              | String | The name of the vectorizer module to use (e.g., `text2vec-cohere`). Set to `none` to disable auto-vectorization and provide vectors manually.                              | Module-specific default | No            |
| `vectorIndexType`                         | String | The type of vector index to use. Common values are `hnsw` (default), `flat`, or `dynamic`.                                                                                 | `hnsw`                  | No            |
| `vectorIndexConfig`                       | Object | An object containing configuration settings specific to the chosen `vectorIndexType` (e.g., HNSW parameters like `ef`, `maxConnections`).                                  | Index-specific defaults | Partially\*   |
| `vectorConfig`                            | Object | A parent object used to define one or more named vector spaces. Each key within this object is a unique name for a vector space (e.g., "title_vector").                    | `null`                  | Partially\*\* |
| ↪ `vectorConfig.<name>.vectorizer`        | Object | A vectorizer configuration specific to the named vector. It defines the module and which properties to vectorize (e.g., `{"text2vec-openai": {"properties": ["title"]}}`). | (Required)              | No            |
| ↪ `vectorConfig.<name>.vectorIndexType`   | String | The index type (`hnsw`, `flat`, etc.) for this specific named vector.                                                                                                      | `hnsw`                  | No            |
| ↪ `vectorConfig.<name>.vectorIndexConfig` | Object | The configuration for the index of this specific named vector.                                                                                                             | Index-specific defaults | Partially\*   |

\* See [vector index mutable parameters](#vector-index-mutable-parameters)<br/>
\*\* New named vectors can be added after collection creation

### Vector index mutable parameters {#vector-index-mutable-parameters}

The following vector index configuration parameters can be modified after collection creation:

| Parameter                     | Type    | Description                                            | Default      | Mutable |
| :---------------------------- | :------ | :----------------------------------------------------- | :----------- | :------ |
| `dynamicEfFactor`             | Integer | Dynamic ef factor for HNSW index                       | `8`          | Yes     |
| `dynamicEfMin`                | Integer | Minimum dynamic ef for HNSW index                      | `100`        | Yes     |
| `dynamicEfMax`                | Integer | Maximum dynamic ef for HNSW index                      | `500`        | Yes     |
| `filterStrategy`              | String  | Filter strategy for HNSW index (introduced in v1.27.0) | `acorn`      | Yes     |
| `flatSearchCutoff`            | Integer | Threshold for switching from HNSW to flat search       | `40000`      | Yes     |
| `skip`                        | Boolean | Skip vector indexing for this collection               | `false`      | Yes     |
| `vectorCacheMaxObjects`       | Integer | Maximum number of objects in vector cache              | `1e12`       | Yes     |
| **Binary Quantization (BQ)**  |         |                                                        |              |         |
| `bq.enabled`                  | Boolean | Enable binary quantization                             | `false`      | Yes     |
| `bq.rescoreLimit`             | Integer | Number of candidates to rescore for BQ                 | `200`        | Yes     |
| **Product Quantization (PQ)** |         |                                                        |              |         |
| `pq.enabled`                  | Boolean | Enable product quantization                            | `false`      | Yes     |
| `pq.centroids`                | Integer | Number of centroids for PQ                             | `256`        | Yes     |
| `pq.segments`                 | Integer | Number of segments for PQ                              | `128`        | Yes     |
| `pq.trainingLimit`            | Integer | Training limit for PQ                                  | `100000`     | Yes     |
| `pq.encoder.type`             | String  | Encoder type for PQ                                    | `kmeans`     | Yes     |
| `pq.encoder.distribution`     | String  | Distribution for PQ encoder                            | `log-normal` | Yes     |
| **Scalar Quantization (SQ)**  |         |                                                        |              |         |
| `sq.enabled`                  | Boolean | Enable scalar quantization                             | `false`      | Yes     |
| `sq.rescoreLimit`             | Integer | Number of candidates to rescore for SQ                 | `200`        | Yes     |
| `sq.trainingLimit`            | Integer | Training limit for SQ                                  | `100000`     | Yes     |
...
| Parameter                | Type    | Description                                    | Default | Mutable |
| :----------------------- | :------ | :--------------------------------------------- | :------ | :------ |
| `bm25.b`                 | Float   | BM25 b parameter                               | `0.75`  | Yes     |
| `bm25.k1`                | Float   | BM25 k1 parameter                              | `1.2`   | Yes     |
| `cleanupIntervalSeconds` | Integer | Cleanup interval in seconds                    | `60`    | Yes     |
| `stopwords.preset`       | String  | Preset stopword list                           | `en`    | Yes     |
| `stopwords.additions`    | Array   | Additional custom stopwords                    | `[]`    | Yes     |
| `stopwords.removals`     | Array   | Stopwords to remove from preset                | `[]`    | Yes     |
| `indexTimestamps`        | Boolean | Maintain inverted indexes by object timestamps | `false` | No      |
| `indexNullState`         | Boolean | Maintain inverted indexes for null state       | `false` | No      |
| `indexPropertyLength`    | Boolean | Maintain inverted indexes by property length   | `false` | No      |
...
| `asyncEnabled`     | Boolean | Enable asynchronous replication (introduced in v1.26.0)                                                         | `false`                   | Yes                                   |
| `deletionStrategy` | String  | Strategy for handling deletions in replication (introduced in v1.27.0)                                          | `"NoAutomatedResolution"` | Yes                                   |
...
| `autoTenantCreation`   | Boolean | If `true`, a new tenant is created if you try to insert an object into a non-existent tenant. (introduced in v1.25.0)                                            | `false` | Yes     |
| `autoTenantActivation` | Boolean | If `true`, automatically activate `INACTIVE` or `OFFLOADED` tenants if a search, read, update, or delete operation is performed on them. (introduced in v1.25.2) | `false` | Yes     |
...
After you create a collection, you can [add new properties](./properties.mdx#adding-a-property-after-collection-creation). You cannot modify existing properties after you create the collection. You can also [add new named vectors](./vectorization.mdx#adding-a-named-vector-after-collection-creation).
