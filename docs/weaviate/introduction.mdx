...
* Weaviate allows you to store and retrieve data objects based on their semantic properties by indexing them with [vectors](./concepts/indexing/vector-index.md).
...
Weaviate's [vector indexing mechanism is modular](./concepts/indexing/vector-index.md), and the current available plugin is the Hierarchical Navigable Small World (HNSW) multilayered graph.
