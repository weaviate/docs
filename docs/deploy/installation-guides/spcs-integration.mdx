...
:::warning Set your own password
Replace `<YOUR_STRONG_PASSWORD>` with a strong password before you run this. `MUST_CHANGE_PASSWORD` is `FALSE` because the service signs in with these credentials, so nothing prompts you to replace a weak password later.
:::
