...
| `DISABLE_STARTUP_BANNER` | Disable the banner Weaviate logs shortly after startup (`action=banner`, with the version, the link to [Improve your cluster](/improve-your-cluster), and this node's `/v1/meta` URL), and its repeat every `BANNER_INTERVAL`. The banner is part of telemetry, so a cluster with `DISABLE_TELEMETRY=true` never logs one. It is an `info` entry, so `LOG_LEVEL=warning` or stricter hides it as well. Default: `false`<br/>Added in `v1.40` | `boolean` | `true` |
