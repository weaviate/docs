...
| `BANNER_INTERVAL` | How often the banner is logged again after its first appearance. The banner draws its art from `https://weaviate.io/banner/v1.json` when it can be fetched, and its link carries `?clusterid=<uuid>`. Banner and fetch happen only when telemetry is enabled, since the id exists only then. Default: `24h`<br/>Added in `v1.40` | `string - duration` | `12h` |
...
| `DISABLE_STARTUP_BANNER` | Disable the banner Weaviate logs once the cluster has an id (`action=banner`, with the version, the link to [Improve your cluster](/improve-your-cluster) carrying `?clusterid=<uuid>`, and this node's `/v1/meta` URL), and its repeat every `BANNER_INTERVAL`. The banner needs the id, so a cluster with `DISABLE_TELEMETRY=true` never logs one. It is an `info` entry, so `LOG_LEVEL=warning` or stricter hides it as well. Default: `false`<br/>Added in `v1.40` | `boolean` | `true` |
...
| `DOCS_BASE_URL` | Host that the links in log entries (`docs_url`), in error messages and in the startup banner point at. Set it to a docs mirror in an environment without internet access, or to a preview while docs are in review; only `http` and `https` are accepted, and an invalid value is logged and ignored. Default: `https://docs.weaviate.io`<br/>Added in `v1.40` | `string - URL` | `http://docs.internal` |
