...
| `HNSW_STARTUP_WAIT_FOR_VECTOR_CACHE` | **Deprecated in `v1.36.6`.** If `true`, vector cache prefill is synchronous when a node starts. The node reports ready to serve when the cache is hot. Now defaults to `true` and is automatically set to `false` when lazy loading is enabled. | `boolean` | `true` |
