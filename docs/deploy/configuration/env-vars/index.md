...
| `BACKUP_CHUNK_TARGET_SIZE` | Target size of a backup chunk when Weaviate packs several small shard files into one chunk. Weaviate raises the effective value to at least the big-file threshold derived from `BACKUP_MIN_CHUNK_SIZE`. This is a server-level setting and is separate from the per-request [`ChunkSize`](/deploy/configuration/backups.md#available-config-object-properties) option. Accepts a plain byte count or a value with a unit (`B`, `KiB`, `MiB`, `GiB`, `TiB`, `KB`, `MB`, `GB`, `TB`). Default: `10MiB`<br/>Added in `v1.36`, and backported to `v1.33.14`, `v1.34.11`, and `v1.35.4` | `string` | `32MiB` |
| `BACKUP_MIN_CHUNK_SIZE` | Lower bound for the big-file threshold used when chunking a backup. A shard file at or above the threshold is written to its own chunk, which lets an incremental backup skip it when it has not changed; smaller files are packed together. This is a server-level setting and is separate from the per-request [`ChunkSize`](/deploy/configuration/backups.md#available-config-object-properties) option. Accepts a plain byte count or a value with a unit (`B`, `KiB`, `MiB`, `GiB`, `TiB`, `KB`, `MB`, `GB`, `TB`). Default: `1MiB`<br/>Added in `v1.37`, and backported to `v1.34.18`, `v1.35.13`, and `v1.36.3` | `string` | `4MiB` |
| `BACKUP_SPLIT_FILE_SIZE` | Size at which a single large shard file is split across several backup chunks. This is a server-level setting and is separate from the per-request [`ChunkSize`](/deploy/configuration/backups.md#available-config-object-properties) option. Accepts a plain byte count or a value with a unit (`B`, `KiB`, `MiB`, `GiB`, `TiB`, `KB`, `MB`, `GB`, `TB`). Default: `50GiB`<br/>Added in `v1.37`, and backported to `v1.35.15` and `v1.36.5` | `string` | `10GiB` |
...
| Variable | Description | Type | Example Value |
| --- | --- | --- | --- |
| `AUTHENTICATION_ANONYMOUS_ACCESS_ENABLED` | Allow users to interact with weaviate without auth | `boolean` | `true` <br/> Defaults to `true` |
