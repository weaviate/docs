import asyncio
import uuid

from engram import APIError

# START Connect
import os
from engram import AsyncEngramClient

client = AsyncEngramClient(api_key=os.environ["ENGRAM_API_KEY"])
# END Connect

test_user_id = f"test-{uuid.uuid4().hex[:8]}"


async def main():
    # Setup: store a memory so we can get and delete it
    run = await client.memories.add(
        "The user prefers dark mode",
        user_id=test_user_id,
        group="default",
    )
    status = await client.runs.wait(run.run_id)
    assert status.status == "completed"
    assert status.committed_operations is not None
    assert len(status.memories_created) >= 1

    memory_id = status.memories_created[0].memory_id

    # START GetMemory
    memory = await client.memories.get(
        memory_id,
        user_id=test_user_id,
        group="default",
    )

    print(memory.content)
    print(memory.topic)
    # END GetMemory

    assert memory.id == memory_id
    assert "dark mode" in memory.content

    # START DeleteMemory
    await client.memories.delete(
        memory_id,
        user_id=test_user_id,
        group="default",
    )
    # END DeleteMemory

    # Verify the memory was deleted
    try:
        await client.memories.get(memory_id, user_id=test_user_id, group="default")
        assert False, "Expected memory to be deleted"
    except APIError as e:
        assert e.status_code == 404, f"Expected 404 after delete, got {e.status_code}"

    # Cleanup
    _all = await client.memories.search(query="dark mode", user_id=test_user_id, group="default")
    for _m in _all:
        await client.memories.delete(_m.id, user_id=test_user_id, group="default")

    await client.aclose()


asyncio.run(main())
