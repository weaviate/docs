...
async def main():
    ...
    bm25_no_overlap = await client.memories.search(
    ...
    try:
        nonexistent_topic_results = await client.memories.search(
            query="user preferences",
    ...
    # Cleanup
    _all = await client.memories.search(query="user", user_id=test_user_id)
    for _m in _all:
