...
If you are using WCD, a `qna` module is enabled by default ([see the documentation](/wcs#configuration)). Otherwise, you must configure your Weaviate instance to make sure that a `qna-xxx` module is enabled.

This is outside the scope of this unit, but you can refer to the [module configuration](/weaviate/modules/reader-generator-modules/index.md) for information on how to configure each module.
