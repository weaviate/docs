...
Weaviate's default metric is cosine, but you can also use any number of [other available metrics](/docs/weaviate/config-refs/distances.md).
...
- `PERSISTENCE_HNSW_MAX_LOG_SIZE`: Maximum size of the HNSW [write-ahead-log](/docs/weaviate/concepts/storage.md#hnsw-vector-index-storage). Increase this to improve log compaction efficiency, or decrease to reduce memory requirements.
...
- [References: Vector index parameters](/docs/weaviate/config-refs/schema/vector-index.md)
- [How-to manage collections](/docs/weaviate/manage-data/collections.mdx)
