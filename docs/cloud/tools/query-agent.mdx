...
The **Weaviate Cloud (WCD) Query Agent** provides an interactive console for using the Weaviate [Query Agent](/query-agent/index.md), a pre-built agentic service designed to answer natural language queries based on the data stored in Weaviate Cloud.
...
The WCD Query Agent does not support multi-tenancy. In order to utilize multi-tenancy with the Query Agent, use a [client library](/query-agent/reference/advanced_collections.md).
