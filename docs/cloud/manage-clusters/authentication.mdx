...
A new cluster ships with no API keys — create one before you connect. This applies to free and Shared Cloud clusters alike, and both can create, edit, rotate and delete keys.
...
2.  Navigate to the `API Keys` section of the cluster `Dashboard`. A new cluster shows the empty state `There are no API keys for this cluster yet`.
