...
For collections using the flat index:

<Tabs className="code" groupId="languages">
  <TabItem value="py" label="Python">
      <FilteredTextBlock
        text={PyCode}
        startMarker="# START UpdateSingleCollectionFlat"
        endMarker="# END UpdateSingleCollectionFlat"
        language="py"
      />
  </TabItem>
  <TabItem value="ts" label="JavaScript/TypeScript">
...
For collections using the dynamic index (which can switch between flat and HNSW), configure compression for both underlying index types:
...
The following example enables RQ compression on all HNSW collections listed from above:
...
If you have many large collections, avoid enabling compression for multiple collections simultaneously. The cumulative memory overhead can exhaust cluster resources.
...
2. **Process one at a time**: For large collections, enable compression on one collection (or one named vector) at a time
3. **Build in buffers**: If using a loop, add sufficient delays between operations to allow each compression to complete before starting the next
4. **Start with smaller collections**: Test on smaller collections first to understand timing and resource impact before compressing larger ones
