...
- **[Sandbox clusters](#sandbox-clusters)**: Sandbox clusters are small, free clusters designed for learning and experimentation. Sandbox clusters are not scalable and expire after 14 days. You will also have an option to extend their lifetime and convert them into a production ready Serverless cluster.
- **[Serverless clusters](#serverless-clusters)**: Serverless clusters are robust, paid clusters designed for production use.
...
4. Select a cloud region from the dropdown.
5. Configure the `Advanced configuration` settings if needed.
...
- [`Enable auto schema generation`](https://www.google.com/search?q=/weaviate/config-refs/collections%23auto-schema)
- [`Enable async indexing`](https://www.google.com/search?q=/weaviate/concepts/vector-index%23asynchronous-indexing)
- [`Enable async replication`](https://www.google.com/search?q=/deploy/configuration/replication%23async-replication-settings)
...
<details>
    <summary>Steps to create a Serverless cluster</summary>

To create a Serverless cluster, follow these steps:

1. In the clusters sidebar, click the plus button.
2. Select the `Serverless` option.
3. Choose a name for your cluster.
4. Select a cloud region from the dropdown.
5.  Optionally, enable `High Availability` to reduce downtime during upgrades or scaling.
6. Configure the `Advanced configuration` settings if needed.
